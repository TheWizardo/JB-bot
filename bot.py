...
def energizer(bot, update):
    ...
    energy = energy.split(" ")
    if len(energy) == 1:
        a = random.randint(1, len(energizers) - 1)
...
def emberes(bot, update):
    ...
    if iknow:
        ...
        index = random.randint(1, len(folders[u'embarrassment']) - 1)
...
def bannana(bot, update):
    ...
    logs.close()
    c = random.randint(1, len(folders[u'Bananas']) - 1)
    name = "ban" + str(c) + ".png"
...
def jbb(bot, update):
    ...
    bot.send_message(chat_id=update.message.chat_id, text=txt)
    txt = "If you need to contact any one of them, use /jb_phone <name>"
    bot.send_message(chat_id=update.message.chat_id, text=txt)
    ...
    if update.message.chat.type != "private":
        ...
        if newbie != None:
            ...
            if iknow:
                ...
                newJB = JBer(EN_name="",HE_name="", Phone="", Roles=["None"], Birthdate="", User_ID=update.message.from_user.id)
...
def lullabies(bot, update):
    ...
    parts = message.split(" ")
    if len(parts) == 1:
        r = random.randint(1, len(lullaby) - 1)
...
def info_gather(bot, update):
    ...
    if str(update.message.chat_id) in newbies:
        ...
        if int(newbies[str(update.message.chat_id)]["stage"]) == 0: #getting the En_Name
            ...
            newbies[str(update.message.chat_id)]["JBer"].set_En_Name(En_Name)
            ...
            newbies[str(update.message.chat_id)]["JBer"].set_He_Name(He_Name)
            ...
            if len(phone) == 10:
                ...
                newbies[str(update.message.chat_id)]["JBer"].set_Phone(phone_num)
